...
incremental_delta_metrics AS (
    ...
    WHERE x.active_year = 2021
      AND EXISTS (
          ...
          WHERE x.actorid = y.actorid AND y.end_date = 2021 AND (x.quality_class <> y.quality_class OR x.is_active <> y.is_active)
      )
),
...
do_insert AS (
    ...
    FROM (
        ...
        SELECT * FROM incremental_delta
        UNION
        SELECT * FROM incremental_delta_metrics
    ) AS combined
